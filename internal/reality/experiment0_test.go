...
func TestBuildExperiment0ReportsFrozenPublicCoverage(t *testing.T) {
	...
	if len(report.PublicValidation.Results) != 6 {
		t.Fatalf("expected six cases, got %d", len(report.PublicValidation.Results))
	}
	...
	if len(report.ContinuityCoverage[string(LineWorkspace)]) != 3 || len(report.ContinuityCoverage[string(LineConversation)]) != 4 {
		t.Fatalf("unexpected continuity coverage: %#v", report.ContinuityCoverage)
	}
	if len(report.ContinuityCoverage[string(LineBridge)]) != 2 {
		t.Fatalf("expected two bridge cases, got %#v", report.ContinuityCoverage)
	}
	if len(report.PressureCoverage["explicit_deletion"]) != 1 {
		t.Fatalf("expected deletion pressure coverage: %#v", report.PressureCoverage)
	}
	if report.EvidenceLevels[string(EvidencePublic)] != 6 || report.SealedStatus != "unavailable" {
